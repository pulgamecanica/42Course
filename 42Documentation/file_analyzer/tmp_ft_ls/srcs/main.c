...
static ls_config	init_config() {
	...
	config.files = NULL;
	config.format = multiple_column_format;
	config.sorting = name_sort;
	ioctl(STDOUT_FILENO, TIOCGWINSZ, &config.sz);
    if (DEBUG)
	   ft_printf("PROGRAM SIZE: [%d, %d]\n", config.sz.ws_col, config.sz.ws_row);
	return config;
}
...
int	main(int ac, char * av[]) {
	 ...
	 **/
	ft_print_regular_files(&config); //print argument files
