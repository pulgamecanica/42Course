...
void f(t_conf * conf, t_list ** param_files, t_list ** pending_directories) {
	...
	conf->params_on = false; // AFTER PRINTING PARAMETER FILES WE SHOULD TURN THIS OFF
	if (pending_directories && ft_lstsize(*pending_directories)) {
...
int	main(int ac, char *av[]) {
	...
	if (init_program(param_files, conf, ac, av) != EXIT_SUCCESS) // READ ALL THE INPUT, ADDING PARAMETER FILES AND FLAGS
