  ...
  switch (notification) {
    ...
    case StudentRequestToJoinCourse : {
      ...
      if (course) {
        ...
        std::cout << "Sorry " << *sender << " but there are not courses right now for you to learn, wait until a professor opens subscriptions." << std::endl;
    } return ;
    case ProfessorRequestNewCourse : {
      ...
      ncc->FillCourse(prof->GetName() + "'s Course", 9, 3, prof); /* Can you full this form */
...
void Headmaster::RingBell() {
  ...
  speakers_.Notify(Event::RingBell);
  if (DEBUG)
    std::cout << *this << GREEN << " [RING BELL]\t" << ENDC << std::endl;
}

void Headmaster::FoodTruckArrived() {
  std::cout << BLUE << "        ______________________" << ENDC << std::endl;
  std::cout << BLUE << "       |                       |" << ENDC << std::endl;
  std::cout << BLUE << "       | -T A C O S ($ 5)   ___|" << ENDC << std::endl;
  std::cout << BLUE << "       | -S O U P ($ 10)    |----." << ENDC << std::endl;
  std::cout << BLUE << "       | -H O T D O G ($ 8) | |O,|____" << ENDC << std::endl;
  std::cout << BLUE << "  @    |  .-.          .-.  | -  .-.  |" << ENDC << std::endl;
  std::cout << BLUE << "  `@@ =(_| @ |________| @ |_|___| @ |_) " << ENDC << std::endl;
  std::cout << BLUE << "          `-'          '-'       `-'" << ENDC << std::endl;
  speakers_.Notify(Event::FoodTruckHorn);
  if (DEBUG)
    std::cout << *this << GREEN << " [FOOD TRUCK ARRIVED]\t" << ENDC << " yay, go eat lunch!" << std::endl;
}

void Headmaster::SetObserver(std::shared_ptr<IObserver> observer) {
  speakers_.Attach(observer);
}
...
void Headmaster::SetObservers() {
  for(auto& staff : StaffList::GetInstance()->getItems())
    if (std::dynamic_pointer_cast<Professor>(staff))
      SetObserver(std::dynamic_pointer_cast<Professor>(staff));
