...
Person::Person(const std::string & name, std::shared_ptr<IMediator> mediator): name_(name), mediator_(mediator) {
  if (DEBUG)
...
void Person::SetMediator(std::shared_ptr<IMediator> mediator) {
  mediator_ = mediator;
  if (DEBUG)
    std::cout << *this << GREEN << " [SET MEDIATOR]\t" << ENDC << std::endl;
}

std::shared_ptr<Room> Person::GetCurrentRoom() const {
