...
- [Examples](#examples)
- [Options](#options)
- [Optimal Train Travel Time Calculator](#optimal-train-travel-time-calculator)
...
| Can there be two rails between two points (two trains traveling using the same path each on his own rail) | There is no such logic, there is only one rail per edge, from A-B / B-A |
...
| Is the distance in KM and the speed in KM/H? | No, the distance is in KM and the forces are in m/s |
| Should I set a max speed (LIMIT)? | Yes, even the subject mentions this, though you can change this in the settings to run different simulations |
...
| Should I use multithreading? | For doing the GUI it's a good idea, but I wont... |
...
| How is overtaking done in real life? | Good question... :O | I've never seen a train over take another necessarily... |
...
| In case of an event, should the train stop right there, or wait the given amount at the station next available (City)? Maybe this depends. | Events will only affect on the Nodes / Vertices | 
| What happens if there is no way to get to the destination? | Display error on the logfile and stay put |
| How often should the system update(fps)? | Probably each second or every 1 second |
| Can trains wait in a node and other trains pass by? (is this overtaking) | Exactly |
| Can Trains block the rails? | Yes, when slower than the one behind |
| Can trains calculate their paths on runtime? (after they started their journey, in case another train is blocking the way?) | Yes |
| Should I calculate the REAL time when finding the path | Yes |
| What is the find path algorithm? | Find the shortest path |
| Should the network be 100% connected? | Not necesarilly |
| Can there be "Broken" rails "Out of service" (lol)? | Nope |
| Should Trains stop at each station? | Yes |
| Are events intoduced while the simulation is going on? | No, events are launched before the simulation and are applied according to their probability, events are launched for each train, when the train passes in that segment, events apply to trains only, not the external resources |