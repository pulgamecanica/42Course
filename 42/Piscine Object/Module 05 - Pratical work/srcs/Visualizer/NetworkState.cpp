...
NetworkState::NetworkState(SimulationsEngine& engine)
  : engine_(engine), 
    grid_(engine.GetRailwaySystem(), 20.0f, {50, 50, GetScreenWidth() - 100, GetScreenHeight() - 100}) {
  float screenHeight = GetScreenHeight();
  float screenWidth = GetScreenWidth();
  button_manager_.AddButton("Home", {5, 5, 50, 25}, [this]() { engine_.ChangeState(EngineStates::MENU); });
  button_manager_.AddButton("Show Grid", {screenWidth - 105, 5, 100, 25}, [this]() { grid_.ToggleShowGrid(); });
}
