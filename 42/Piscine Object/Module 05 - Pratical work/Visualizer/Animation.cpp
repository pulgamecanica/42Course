...
Animation::Animation(const std::vector<const char*>& imagePaths, float frameDuration, std::map<std::string, int> options) {
  ...
  bool resize_width = options.contains("width");
  bool resize_height = options.contains("height");
  ...
  for (const char* path : imagePaths) {
    ...
    if (resize_width || resize_height)
      ImageResizeNN(&img,
        resize_width ? options["width"] : img.width,
        resize_height ? options["height"] : img.height);
